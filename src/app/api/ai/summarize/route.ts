...
export async function POST(req: Request) {
  ...
  try {
    const { data: interviewSession, error: sessionError } = await supabaseAdmin
      .from("sessions")
      .select(
    ...
    if (sessionError) {
      log.error("Failed to load session for summary:", sessionError.message);
      return NextResponse.json(
        { error: `Could not load interview session: ${sessionError.message}` },
        { status: 500 },
      );
    }
    ...
    log.info(`Generating report with ${provider.id}/${REPORT_MODEL}`);
    ...
    if (updateError) {
      throw new Error(`Failed to save generated report: ${updateError.message}`);
    }
