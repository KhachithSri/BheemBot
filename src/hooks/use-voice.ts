...
/**
 * Voice interview hook using the local voice relay.
 *
 * Flow:
 * 1. Browser connects and sends interview context to relay
 * 2. Relay builds the system prompt and connects to the local speech service
 * 3. Browser captures mic audio as 16kHz int16 PCM
 * 4. Audio sent to relay server via WebSocket (hex-encoded)
 * 5. Relay forwards audio to the local speech service for ASR + LLM + TTS
...
export function useVoice({
  ...
  /** Extract text from a relay event payload, trying common field names */
  ...
  const handleRelayMessage = useCallback(
    ...
    (msg: Record<string, any>) => {
      switch (msg.type) {
        ...
        case "asr": {
          // ASR transcript chunk from the relay — accumulate.
          const results =
