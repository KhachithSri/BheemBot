...
Provide a structured analysis as VALID JSON ONLY (use only standard ASCII double-quotes, never Unicode smart quotes like \u201C \u201D):
{
  ...
  // Groq requires at least one user message, including for text-only reports.
  if (hasDrawings) {
  ...
  result.push({
    role: "user",
    content: "Analyze the interview transcript and return the requested report as valid JSON.",
  });
