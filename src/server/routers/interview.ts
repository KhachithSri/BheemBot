    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (projectError) {
        throw new TRPCError({
          ...
          message: `Database error checking project: ${projectError.message}. Ensure migrations are applied.`,
        });
      }
      ...
      if (error) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: `Failed to create interview: ${error.message}. This usually means the database schema is missing columns. Run migrations in Supabase SQL Editor.`,
        });
      }
