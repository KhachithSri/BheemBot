...
test("login honors browser locale and persisted locale cache", async () => {
  ...
  await cachedPage.addInitScript(() => {
    window.localStorage.setItem("Bheem.app.locale", "zh");
  });
...
test("mobile auth callback page hands OAuth params back to the app", async () => {
  ...
  await waitForText(page, "Returning you to Bheem");

  const expected =
    "com.Bheem.mobile://login-callback?code=oauth-code&state=xyz";

  await waitForCondition(async () => {
    const href = await page
      .getByRole("link", { name: "Open Bheem" })
      .getAttribute("href");
    return href === expected;
  });
  assert.equal(
    await page.getByRole("link", { name: "Open Bheem" }).getAttribute("href"),
    expected,
  );
